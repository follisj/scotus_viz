scotus_agree_5 <- function(ind,cons,data,color.palette){
  ind=ind
  
  ## generate agreement rates for labels
  x.lab0 <- j.agree2(ind,data %>% select(ind,cons),0) %>%
    mutate(agree=paste0("None ",pct_agree,"%"))
  ...
  x.lab5 <- j.agree2(ind,data %>% select(ind,cons),5) %>%
    mutate(agree=paste0("All ",pct_agree,"%"))
  
  ggplot(x.lab1,aes(x=1,y=rep(.2,5)))+
    geom_col(aes(fill=pct_agree),col="white",width=2)+
    
    # single justice agreement
    geomtextpath::geom_textpath(aes(x=rep(1.85,5),y=seq(.1,.9,.2),label=agree),angle=90,size=8,alpha=.75)+
    geom_rect(xmin=0,xmax=1.7,ymin=-Inf,ymax=Inf,fill="gray90")+
    
    # 2 justice agreement
    geom_col(data=x.lab2,aes(x=1.45,y=rep(.1,10),fill=pct_agree),col="white",width=.3)+
    geomtextpath::geom_textpath(data=x.lab2,aes(x=rep(1.45,10),y=seq(.05,.95,.1),label=agree),angle=90,size=4,alpha=.6)+
    
    # 3 justice agreement
    geom_col(data=x.lab3,aes(x=1.05,y=rep(.1,10),fill=pct_agree),col="white",width=.3)+
    geomtextpath::geom_textpath(data=x.lab3,aes(x=rep(1.05,10),y=seq(.05,.95,.1),label=agree),angle=90,size=4,alpha=.6)+
    
    # 4 justice agreement
    geom_col(data=x.lab4,aes(x=.65,y=rep(.2,5),fill=pct_agree),col="white",width=.3)+
    geomtextpath::geom_textpath(data=x.lab4,aes(x=rep(.65,5),y=seq(.1,.9,.2),label=agree),angle=90,size=4,alpha=.6)+
    
    # center labels
    geom_col(data=data.frame(x=c(.2,.2),y=c(.5,.5),
                             agree=c(x.lab5$agree,x.lab0$agree),
                             pct_agree=c(x.lab5$pct_agree,x.lab0$pct_agree)),
    ...
    geomtextpath::geom_textpath(data=data.frame(x=c(1.85,1.85),y=c(.12,.88),label=c("Lowest  Agreement","Highest  Agreement")),
